...
export default defineConfig([
	// .github/workflows/scripts/* are out-of-band CI glue (run via tsx from a dot-dir the tsconfig project
	// doesn't cover): they use node:fs + relative imports on purpose, not the wrapper/alias — so keep them
	// out of linting rather than forcing source-tree conventions onto them.
	globalIgnores(["archive/**", "components/monaco-vscode-api/**", ".github/workflows/scripts/**", "test/runtime/deno.ts", "util/bottleneck.ts"]),
	...config,
